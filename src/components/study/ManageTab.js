"use client";
// Admin-only question bank import/management — lifted from the old
// standalone /qbank route. No internal admin check here — gating happens
// once, in PracticePanel, which is the only thing allowed to render this.
import { useEffect, useState } from "react";
import { UNITS, getUnitTopics } from "@/lib/units";
import { IconTrash } from "@/components/Icons";

export default function ManageTab({ supabase, user, showToast }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 11, color: "var(--amber)", fontWeight: 700, letterSpacing: 1 }}>PRACTICE · ADMIN</div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 12, color: "var(--soft)", marginBottom: 10 }}>
          Paste a JSON array. <code>unit</code>/<code>topic</code> are the exact readable names shown in the app. Optional <code>type</code>: <code>&quot;exam_like&quot;</code> (default) or <code>&quot;official_exam&quot;</code>.
        </div>
        <textarea
          value={importText}
          onChange={(e) => setImportText(e.target.value)}
          placeholder='[{"unit":"Cardio-Respiratory","topic":"Thoracic wall","type":"official_exam","question":"...","options":[{"text":"...","correct":true},{"text":"...","correct":false}]}]'
          rows={7}
